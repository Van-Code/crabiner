...
export async function notifyNewReply(replyId, toUserId, fromUserId) {
  try {
    ...
    await query(
      `INSERT INTO notifications (user_id, type, entity_id, title, body, data)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [
        toUserId,
        "new_reply",
        ...
        title,
        body,
        JSON.stringify({ fromUserId, replyId }),
      ]
    );
...
export async function getUserNotifications(
  ...
  let sql = `
    SELECT id, type, entity_id, title, body, data, created_at, read, read_at
    FROM notifications
    WHERE user_id = $1
  `;
